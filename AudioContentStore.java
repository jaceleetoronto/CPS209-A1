...
public class AudioContentStore
{
    ...
    public ArrayList<AudioContent> getContentsList()
    {
        ArrayList<AudioContent> fullList = new ArrayList<AudioContent>();
        for (int i = 0; i < contents.size(); i++) {
            fullList.add(contents.get(i));
        }
        return fullList;
    }

    public ArrayList<Song> getFullSongList()
    {
        ArrayList<Song> fullSongList = new ArrayList<Song>();
        for (int i = 0; i < contents.size(); i++) {
            if (contents.get(i).getType() == Song.TYPENAME){
                fullSongList.add((Song)contents.get(i));
            }
        } return fullSongList;
    }
